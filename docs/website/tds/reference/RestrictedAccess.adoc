...
== Certificate-Based Authorization and Authentication

If clients need to be authenticated to the server, then client-side
certificates are required. Currently, this can only be enabled at the
Tomcat or Apache level. See the appropriate Apache documentation on
how this is enabled.
